...
sed --in-place --expression "s/CCC_CTWO/  c2  /g"                   $1 # c2
sed --in-place --expression "s/CCC_FOURF/  4f  /g"                  $1 # 4f
sed --in-place --expression "s/CCC_TWOSIX/  26  /g"                 $1 # 26
sed --in-place --expression "s/CCC_TWOTHREE/  23  /g"               $1 # 23
sed --in-place --expression "s/CCC_TWOTHREE/  17  /g"               $1 # 17
sed --in-place --expression "s/CCC_TWOTHREE/  16  /g"               $1 # 16
sed --in-place --expression "s/CCC_FIFTEENH/  0xf  /g"              $1 # 0xf
sed --in-place --expression "s/CCC_FOURTEENH/  0xe  /g"             $1 # 0xe
sed --in-place --expression "s/CCC_THIRTEENH/  0xd  /g"             $1 # 0xd
sed --in-place --expression "s/CCC_TWELVEH/  0xc  /g"               $1 # 0xc
sed --in-place --expression "s/CCC_ELEVENH/  0xb  /g"               $1 # 0xb
sed --in-place --expression "s/CCC_TENH/  0xa  /g"                  $1 # 0xa
sed --in-place --expression "s/CCC_NINEH/  0x9  /g"                 $1 # 0x9
sed --in-place --expression "s/CCC_EIGHTH/  0x8  /g"                $1 # 0x8
sed --in-place --expression "s/CCC_SEVENH/  0x7  /g"                $1 # 0x7
sed --in-place --expression "s/CCC_SIXH/  0x6  /g"                  $1 # 0x6
sed --in-place --expression "s/CCC_FIVEH/  0x5  /g"                 $1 # 0x5
sed --in-place --expression "s/CCC_FOURH/  0x4  /g"                 $1 # 0x4
sed --in-place --expression "s/CCC_THREEH/  0x3  /g"                $1 # 0x3
sed --in-place --expression "s/CCC_TWOH/  0x2  /g"                  $1 # 0x2
sed --in-place --expression "s/CCC_ONEH/  0x1  /g"                  $1 # 0x1
sed --in-place --expression "s/CCC_ZEROH/  0x0  /g"                 $1 # 0x0
